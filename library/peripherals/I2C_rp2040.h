...
namespace hal::peripherals::i2c {
    ...
    public:
        ...
        std::size_t readfrom(const uint8_t addr, const uint8_t reg, uint8_t *buffer, const size_t length, bool nostop) {
            ...
            i2c_read_blocking(hal_to_rp2040_inst(m_instance), addr, buffer, length, nostop);
        }

        std::size_t writeto(const uint8_t addr, const uint8_t reg, const uint8_t * const buffer, const size_t length, bool nostop) {

            selectRegister(addr, reg);
            i2c_write_blocking(hal_to_rp2040_inst(m_instance), addr, buffer, length, nostop);
        }
        ...
        void setMode(const uint8_t addr, const peripherals::i2c::Mode mode) override {
    ...
    protected:
        ...
        //                    Constructors and Destructor
        // ****************************************************************
        explicit I2C(const peripherals::I2CInstance instance) : InterfaceI2C(), m_inited{false} {
